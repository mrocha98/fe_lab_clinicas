import 'package:fe_lab_clinicas_core/src/logger/logger.dart';
...
void main() {
  group('LoggerImpl', () {
    ...
    group('level', () {
      test('should be LogLevel.trace by default', () {
        expect(sut.level, LogLevel.trace);
      });
    ...
    group('.trace', () {
      setUp(() {
        when(() => loggerPkg.t(any<dynamic>())).thenAnswer((_) {});
      });
      ...
      test('should not be able to log when level is silent', () {
        ...
        verifyNever(() => loggerPkg.t('message'));
      });
      ...
      test('should not be able to log when level is debug', () {
        sut
      ...
      test('should be able to log when level is trace', () {
        sut
          ..level = LogLevel.trace
          ..trace('message');

        verify(() => loggerPkg.t('message')).called(1);
      });
    });
